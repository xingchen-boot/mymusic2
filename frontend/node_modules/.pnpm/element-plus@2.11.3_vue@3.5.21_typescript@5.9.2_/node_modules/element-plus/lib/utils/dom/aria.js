'use***REMOVED***strict';

Object.defineProperty(exports,***REMOVED***'__esModule',***REMOVED***{***REMOVED***value:***REMOVED***true***REMOVED***});

const***REMOVED***FOCUSABLE_ELEMENT_SELECTORS***REMOVED***=***REMOVED***`a[href],button:not([disabled]),button:not([hidden]),:not([tabindex="-1"]),input:not([disabled]),input:not([type="hidden"]),select:not([disabled]),textarea:not([disabled])`;
const***REMOVED***isHTMLElement***REMOVED***=***REMOVED***(e)***REMOVED***=>***REMOVED***{
***REMOVED******REMOVED***if***REMOVED***(typeof***REMOVED***Element***REMOVED***===***REMOVED***"undefined")
***REMOVED******REMOVED******REMOVED******REMOVED***return***REMOVED***false;
***REMOVED******REMOVED***return***REMOVED***e***REMOVED***instanceof***REMOVED***Element;
};
const***REMOVED***isVisible***REMOVED***=***REMOVED***(element)***REMOVED***=>***REMOVED***{
***REMOVED******REMOVED***const***REMOVED***computed***REMOVED***=***REMOVED***getComputedStyle(element);
***REMOVED******REMOVED***return***REMOVED***computed.position***REMOVED***===***REMOVED***"fixed"***REMOVED***?***REMOVED***false***REMOVED***:***REMOVED***element.offsetParent***REMOVED***!==***REMOVED***null;
};
const***REMOVED***obtainAllFocusableElements***REMOVED***=***REMOVED***(element)***REMOVED***=>***REMOVED***{
***REMOVED******REMOVED***return***REMOVED***Array.from(element.querySelectorAll(FOCUSABLE_ELEMENT_SELECTORS)).filter((item)***REMOVED***=>***REMOVED***isFocusable(item)***REMOVED***&&***REMOVED***isVisible(item));
};
const***REMOVED***isFocusable***REMOVED***=***REMOVED***(element)***REMOVED***=>***REMOVED***{
***REMOVED******REMOVED***if***REMOVED***(element.tabIndex***REMOVED***>***REMOVED***0***REMOVED***||***REMOVED***element.tabIndex***REMOVED***===***REMOVED***0***REMOVED***&&***REMOVED***element.getAttribute("tabIndex")***REMOVED***!==***REMOVED***null)***REMOVED***{
***REMOVED******REMOVED******REMOVED******REMOVED***return***REMOVED***true;
***REMOVED******REMOVED***}
***REMOVED******REMOVED***if***REMOVED***(element.tabIndex***REMOVED***<***REMOVED***0***REMOVED***||***REMOVED***element.hasAttribute("disabled")***REMOVED***||***REMOVED***element.getAttribute("aria-disabled")***REMOVED***===***REMOVED***"true")***REMOVED***{
***REMOVED******REMOVED******REMOVED******REMOVED***return***REMOVED***false;
***REMOVED******REMOVED***}
***REMOVED******REMOVED***switch***REMOVED***(element.nodeName)***REMOVED***{
***REMOVED******REMOVED******REMOVED******REMOVED***case***REMOVED***"A":***REMOVED***{
***REMOVED******REMOVED******REMOVED******REMOVED******REMOVED******REMOVED***return***REMOVED***!!element.href***REMOVED***&&***REMOVED***element.rel***REMOVED***!==***REMOVED***"ignore";
***REMOVED******REMOVED******REMOVED******REMOVED***}
***REMOVED******REMOVED******REMOVED******REMOVED***case***REMOVED***"INPUT":***REMOVED***{
***REMOVED******REMOVED******REMOVED******REMOVED******REMOVED******REMOVED***return***REMOVED***!(element.type***REMOVED***===***REMOVED***"hidden"***REMOVED***||***REMOVED***element.type***REMOVED***===***REMOVED***"file");
***REMOVED******REMOVED******REMOVED******REMOVED***}
***REMOVED******REMOVED******REMOVED******REMOVED***case***REMOVED***"BUTTON":
***REMOVED******REMOVED******REMOVED******REMOVED***case***REMOVED***"SELECT":
***REMOVED******REMOVED******REMOVED******REMOVED***case***REMOVED***"TEXTAREA":***REMOVED***{
***REMOVED******REMOVED******REMOVED******REMOVED******REMOVED******REMOVED***return***REMOVED***true;
***REMOVED******REMOVED******REMOVED******REMOVED***}
***REMOVED******REMOVED******REMOVED******REMOVED***default:***REMOVED***{
***REMOVED******REMOVED******REMOVED******REMOVED******REMOVED******REMOVED***return***REMOVED***false;
***REMOVED******REMOVED******REMOVED******REMOVED***}
***REMOVED******REMOVED***}
};
const***REMOVED***triggerEvent***REMOVED***=***REMOVED***function(elm,***REMOVED***name,***REMOVED***...opts)***REMOVED***{
***REMOVED******REMOVED***let***REMOVED***eventName;
***REMOVED******REMOVED***if***REMOVED***(name.includes("mouse")***REMOVED***||***REMOVED***name.includes("click"))***REMOVED***{
***REMOVED******REMOVED******REMOVED******REMOVED***eventName***REMOVED***=***REMOVED***"MouseEvents";
***REMOVED******REMOVED***}***REMOVED***else***REMOVED***if***REMOVED***(name.includes("key"))***REMOVED***{
***REMOVED******REMOVED******REMOVED******REMOVED***eventName***REMOVED***=***REMOVED***"KeyboardEvent";
***REMOVED******REMOVED***}***REMOVED***else***REMOVED***{
***REMOVED******REMOVED******REMOVED******REMOVED***eventName***REMOVED***=***REMOVED***"HTMLEvents";
***REMOVED******REMOVED***}
***REMOVED******REMOVED***const***REMOVED***evt***REMOVED***=***REMOVED***document.createEvent(eventName);
***REMOVED******REMOVED***evt.initEvent(name,***REMOVED***...opts);
***REMOVED******REMOVED***elm.dispatchEvent(evt);
***REMOVED******REMOVED***return***REMOVED***elm;
};
const***REMOVED***isLeaf***REMOVED***=***REMOVED***(el)***REMOVED***=>***REMOVED***!el.getAttribute("aria-owns");
const***REMOVED***getSibling***REMOVED***=***REMOVED***(el,***REMOVED***distance,***REMOVED***elClass)***REMOVED***=>***REMOVED***{
***REMOVED******REMOVED***const***REMOVED***{***REMOVED***parentNode***REMOVED***}***REMOVED***=***REMOVED***el;
***REMOVED******REMOVED***if***REMOVED***(!parentNode)
***REMOVED******REMOVED******REMOVED******REMOVED***return***REMOVED***null;
***REMOVED******REMOVED***const***REMOVED***siblings***REMOVED***=***REMOVED***parentNode.querySelectorAll(elClass);
***REMOVED******REMOVED***const***REMOVED***index***REMOVED***=***REMOVED***Array.prototype.indexOf.call(siblings,***REMOVED***el);
***REMOVED******REMOVED***return***REMOVED***siblings[index***REMOVED***+***REMOVED***distance]***REMOVED***||***REMOVED***null;
};
const***REMOVED***focusElement***REMOVED***=***REMOVED***(el,***REMOVED***options)***REMOVED***=>***REMOVED***{
***REMOVED******REMOVED***if***REMOVED***(!el***REMOVED***||***REMOVED***!el.focus)
***REMOVED******REMOVED******REMOVED******REMOVED***return;
***REMOVED******REMOVED***let***REMOVED***cleanup***REMOVED***=***REMOVED***false;
***REMOVED******REMOVED***if***REMOVED***(isHTMLElement(el)***REMOVED***&&***REMOVED***!isFocusable(el)***REMOVED***&&***REMOVED***!el.getAttribute("tabindex"))***REMOVED***{
***REMOVED******REMOVED******REMOVED******REMOVED***el.setAttribute("tabindex",***REMOVED***"-1");
***REMOVED******REMOVED******REMOVED******REMOVED***cleanup***REMOVED***=***REMOVED***true;
***REMOVED******REMOVED***}
***REMOVED******REMOVED***el.focus(options);
***REMOVED******REMOVED***if***REMOVED***(isHTMLElement(el)***REMOVED***&&***REMOVED***cleanup)***REMOVED***{
***REMOVED******REMOVED******REMOVED******REMOVED***el.removeAttribute("tabindex");
***REMOVED******REMOVED***}
};
const***REMOVED***focusNode***REMOVED***=***REMOVED***(el)***REMOVED***=>***REMOVED***{
***REMOVED******REMOVED***if***REMOVED***(!el)
***REMOVED******REMOVED******REMOVED******REMOVED***return;
***REMOVED******REMOVED***focusElement(el);
***REMOVED******REMOVED***!isLeaf(el)***REMOVED***&&***REMOVED***el.click();
};

exports.focusElement***REMOVED***=***REMOVED***focusElement;
exports.focusNode***REMOVED***=***REMOVED***focusNode;
exports.getSibling***REMOVED***=***REMOVED***getSibling;
exports.isFocusable***REMOVED***=***REMOVED***isFocusable;
exports.isLeaf***REMOVED***=***REMOVED***isLeaf;
exports.isVisible***REMOVED***=***REMOVED***isVisible;
exports.obtainAllFocusableElements***REMOVED***=***REMOVED***obtainAllFocusableElements;
exports.triggerEvent***REMOVED***=***REMOVED***triggerEvent;
//#***REMOVED***sourceMappingURL=aria.js.map
